...
Fastplotlib + pygfx offscreen render (same semantics as ``OBDGraphWithTiePyQTGraph.py``).
...
Filled regions are drawn as ``pygfx.Mesh`` (triangulated in 2D); colormaps use Matplotlib.  Colors do **not**
need to match other backends pixel-for-pixel.

Requires: fastplotlib, pygfx, numpy, scipy, matplotlib, imageio (for ``Figure.export``).
"""
...
PNG_HEIGHT_PX = 4000
DPI = 100
OUTPUT_PNG_PATH = "OBDGraphWithTieFastplotlib.png"
...
# Must match plot x-range and which ``p`` indices are plotted; ties are filtered to this interval.
VP_P_RANGE = "full"
...
# Colormap or color for tie **lines**.  Set to a Matplotlib colormap name (e.g. "gist_rainbow",
# "hsv", "jet", "turbo", "viridis", "plasma", "inferno", "magma", "cividis", "nipy_spectral", "twilight",
# "coolwarm", "winter", "spring", "summer", "autumn", "wistia") to color by ``TIE_LINE_COLOR_MODE``.
# Set to a simple Matplotlib color string (e.g. "black", "red", "blue", "#ff0000") for a fixed tie color.
# Set to ``""`` (or any falsy value) to disable tie lines entirely.
...
# Stroke thickness in **device pixels** (cosmetic pens; independent of zoom).
TIE_LINE_DEVICE_PX = 1.0
GRAPH_LINE_DEVICE_PX = 0.1
# Plot and PNG export background.
...
def _cmap_rgb(cmap_name: str, t: float) -> tuple[float, float, float]:
    """RGB in [0, 1] from a named Matplotlib colormap at ``t`` in [0, 1]."""
...
def _triangulate_polygon_2d(fx: list[float], fy: list[float]) -> np.ndarray | None:
    """Triangle indices (n_tri, 3) into vertices 0..n-1, or None."""
    n = len(fx)
    if n < 3:
        return None
    pts = np.column_stack(
        [np.asarray(fx, dtype=np.float64), np.asarray(fy, dtype=np.float64)]
    )
    if not np.all(np.isfinite(pts)):
        return None

    def _fan_indices() -> np.ndarray:
        return np.array(
    ...
    if not np.any(inside):
        return _fan_indices()
    return tri.simplices[inside].astype(np.int32, copy=False)


def _add_fill_meshes_merged(
    subplot,
    fill_batches: list[tuple[list[float], list[float], tuple[float, float, float, float]]],
    z_fill: float = -1e-3,
) -> None:
    """One ``pygfx.Mesh`` with per-vertex colors (avoids one GPU mesh per strip)."""
    if not fill_batches:
        return
    ...
    vo = 0
    for fx, fy, rgba in fill_batches:
        tri = _triangulate_polygon_2d(fx, fy)
        if tri is None or tri.size == 0:
            continue
        pts2d = np.column_stack(
            [np.asarray(fx, dtype=np.float32), np.asarray(fy, dtype=np.float32)]
        )
        n = pts2d.shape[0]
        ...
        all_pos.append(pos)
        all_col.append(col)
        all_idx.append(np.asarray(tri, dtype=np.int32) + vo)
        vo += n
    if not all_pos:
        return
    positions = np.vstack(all_pos)
    colors = np.vstack(all_col)
    indices = np.vstack(all_idx)
    geom = pygfx.Geometry(
        positions=positions,
        colors=colors,
        indices=indices,
    )
    mat = pygfx.MeshBasicMaterial(color_mode="vertex")
    mesh = pygfx.Mesh(geom, mat)
    subplot._fpl_graphics_scene.add(mesh)
...
def main() -> None:
    ...
    lw_tie = float(TIE_LINE_DEVICE_PX)
    lw_graph = float(GRAPH_LINE_DEVICE_PX)
            ...
            if ps:
                ...
                for p_left, p_right, ti, tj in strips:
                    ...
                    if len(fx) < 3 or not all(
                        ...
                        continue
                    r, g, b, a = rgba
                    fill_batches.append((fx, fy, (r, g, b, a)))
        ...
        if do_tie_lines:
            for p_use, ti, tj in entries:
                ...
                if np.isfinite(y_bot) and np.isfinite(y_top):
                    r, g, b, a = rgba
                    tie_segments.append(
                        (float(p_use), float(y_bot), float(y_top), (r, g, b, a))
                    )
    ...
    fig = fpl.Figure(
        canvas="offscreen",
        size=(int(PNG_WIDTH_PX), int(PNG_HEIGHT_PX)),
    )
    sp = fig[0, 0]
    sp.background_color = (FIGURE_BACKGROUND,)
    sp.camera.maintain_aspect = False

    _add_fill_meshes_merged(sp, fill_batches)

    if tie_segments:
        n_tie = len(tie_segments)
        seg_pts = np.empty((n_tie * 2, 3), dtype=np.float32)
        seg_cols = np.empty((n_tie * 2, 4), dtype=np.float32)
        z_tie = 1e-4
        for i, (p, yb, yt, c) in enumerate(tie_segments):
            seg_pts[2 * i] = (float(p), float(yb), z_tie)
            seg_pts[2 * i + 1] = (float(p), float(yt), z_tie)
            seg_cols[2 * i] = c
            seg_cols[2 * i + 1] = c
        tie_geom = pygfx.Geometry(positions=seg_pts, colors=seg_cols)
        tie_mat = pygfx.LineSegmentMaterial(
            color_mode="vertex",
            thickness=lw_tie,
            thickness_space="screen",
            aa=False,
        )
        tie_lines = pygfx.Line(tie_geom, tie_mat)
        sp._fpl_graphics_scene.add(tie_lines)
    ...
    for x_arr, y_arr, alpha_n in curve_specs:
        ...
        xyz = np.concatenate(
            [xy, np.full((xy.shape[0], 1), 2e-4, dtype=np.float32)], axis=1
        )
        sp.add_line(
            data=xyz,
            colors=(0.0, 0.0, 0.0, float(alpha_n)),
            thickness=lw_graph,
            uniform_color=True,
        )

    sp.camera.show_rect(vp_p_min, vp_p_max, y_lo, y_hi)
    sp.title = title
    sp.axes.colors = ("k", "k", "k")
    if sp.axes.grids:
        sp.axes.grids.xy.visible = False

    fig.show(autoscale=False, maintain_aspect=False, axes_visible=True)
    ...
    try:
        fig.export(OUTPUT_PNG_PATH)
    except ImportError:
        arr = fig.export_numpy(rgb=True)
        from PIL import Image

        Image.fromarray(arr).save(OUTPUT_PNG_PATH)
